    ...
    const createGroupHandler = async (e: FormEvent<HTMLFormElement>) => {
        ...
        if (!selectedMembersEmails.length) {
            ...
            if (userEmail) {
                ...
                if (groupProfile) {
                    const profileRef = ref(storage, `groups_profiles/${docRef.id}/${docRef.id}.${getFileExt(groupProfile.name)}`);
    ...
    return (
        <>
            ...
            <div className="dark:bg-base bg-secondary lg:dark:bg-secondary lg:rounded-xl lg:shadow-xl lg:border p-6 lg:p-8 h-fit m-auto w-full lg:max-w-lg max-lg:h-svh flex flex-col">
                ...
                <form onSubmit={createGroupHandler} className="pt-5 grow flex flex-col h-[calc(100%-2rem)]">
                    <div>
